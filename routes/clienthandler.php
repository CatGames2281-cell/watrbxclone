...
$router->get('/place/{id}/fetch', function($id){
    ...
    if(isset($_GET["apikey"])){
        ...
        if($gameserver->validate_api_key($apikey)){
            ...
            if($asset){
                header("Location: https://cdn.watrbxclone.onrender.com/" . $asset->fileid);
                die();
            } else {
            ...
            if($asset->prodcategory == 9){
                ...
                if($currentuser){
                    ...
                    if($asset->owner == $currentuser->id){
                        try {
                            header("Content-type: application/octet-stream");
                            $asset = file_get_contents("https://cdn.watrbxclone.onrender.com/" . $asset->fileid);
...
$router->get("/Asset/BodyColors.ashx", function(){
    ...
    die('<roblox xmlns:xmime="http://www.w3.org/2005/05/xmlmime" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="https://www.watrbxclone.onrender.com/roblox.xsd" version="4">
